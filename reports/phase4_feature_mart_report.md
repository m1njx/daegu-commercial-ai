...
2. **다차원 Feature Mart 3종 구축 완료**:
   - **점포 단위(Micro-Level)**: [store_spatial_features.parquet](../data/processed/feature_mart/store_spatial_features.parquet) (118,357건, 39컬럼)
   - **행정동 종합 단위(Meso-Level)**: [commercial_feature_mart_dong.parquet](../data/processed/feature_mart/commercial_feature_mart_dong.parquet) (150개 동, 34컬럼) / [CSV](../data/processed/feature_mart/commercial_feature_mart_dong.csv)
   - **동 × 업종 단위(Macro-Level)**: [commercial_feature_mart_dong_category.parquet](../data/processed/feature_mart/commercial_feature_mart_dong_category.parquet) (1,454개 조합, 21컬럼) / [CSV](../data/processed/feature_mart/commercial_feature_mart_dong_category.csv)
...
[scripts/verify_feature_mart.py](../scripts/verify_feature_mart.py)를 실행하여 추출된 무결성 검증 결과는 다음과 같다.
...
1. [대구 행정동별 점포 밀도 및 도시철도 노선망 지도](../reports/figures/store_density_map.png)
2. [대구 구·군별 10대 업종대분류 점포 수 및 구성비 분포](../reports/figures/industry_distribution_by_district.png)
3. [대구 도시철도 접근성 및 시간대별 유동인구 구조 분석](../reports/figures/transit_accessibility_distribution.png)
4. [대구 상권 부설주차 수용용량 공급 구조 및 상업 밀도 대비 수급 분석](../reports/figures/parking_capacity_vs_stores.png)
