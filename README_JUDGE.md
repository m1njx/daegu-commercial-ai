...
2. **검증 포인트**:
   ...
   - **종합 점수**: **약 84.47점**
   - **사유 설명 확인**: 두 번째 탭 **"🗺️ 지도 보기 · 상세 분석"**에서 범어1동을 선택합니다.
...
1. **상단 탭 이동**:
   - 세 번째 탭인 **"⚖️ 통합 대중교통 모델 vs 도시철도 중심 개선 모델"** 클릭
2. **검증 포인트**:
   - **전체 순위 유사도**: 6개 공식 데모에서 대구 전역 150개 행정동의 Spearman Rank Correlation이 $\rho=0.9915\sim0.9969$로 산출됨을 확인합니다(현재 코드·데이터 재계산 기준).
   - **비역세권 접근성 지표 변화**: 도시철도역이 없는 91개 행정동에서 버스 지표를 포함할 때의 평균 접근성 점수 변화를 현재 조건에서 동적으로 확인합니다.

---

## STEP 6: 132개 자동화 회귀 테스트 검증

새 터미널 창을 열고, 프로젝트에 탑재된 12개 Phase 자동화 테스트 스위트를 실행하여 결측치, 이상치, 가중치 정규화, 회귀 일치성, 미진입 필터 및 데이터·UI 정합성을 검증합니다.

```bash
# 기존 Phase 6~20 117개 + Phase 21 15개 = 총 132개
...
- **Q. 동일한 이름의 버스 정류소는 어떻게 처리하나요?**  
  **A.** 위치 파일의 3,981개 표지판은 150개 전 행정동에 공간 매핑됩니다. 이용량 원천은 정류소명만 제공하므로 같은 이름의 표지판을 300m 근접 공간 군집으로 구분하고, 원거리 동명이인은 별도 군집으로 분리해 이름 단위 이용량을 균등 배분합니다. 위치 파일과 이름이 매칭된 이용량은 임의 중립값 대치 없이 전량 보존합니다.
